...
#  Cron: 0 6 * * * /opt/ip-lookup.sh >> /var/log/auth-monitor/ip-lookup.log 2>&1
#  Auch aufrufbar: /opt/ip-lookup.sh --ip 1.2.3.4
# =============================================================================
...
source "$CONF_FILE"
CACHE_DAYS=7        # IP-Info nach X Tagen neu abfragen
...
lookup_ip() {
    ...
    # API-Abfrage
    local response
    response=$(curl -s -m 10 "https://ipinfo.io/${ip}?token=${IPINFO_TOKEN}" 2>/dev/null)
    ...
    # ASN aus org extrahieren (z.B. "AS12345 Some Hoster GmbH")
    local asn hoster
