...
#  Cron: 1 0 * * * root /opt/auth-monitor/daily-reset.sh >> /var/log/auth-monitor/daily-reset.log 2>&1
# =============================================================================
...
# 3. Statistik
TOTAL_FAILS=$(mysql_exec "SELECT COALESCE(SUM(fail_count),0) FROM daily_stats WHERE stat_date='$YESTERDAY' AND server='$SERVER_NAME';" | tail -1)
TOTAL_IPS=$(mysql_exec "SELECT COUNT(DISTINCT ip) FROM daily_stats WHERE stat_date='$YESTERDAY' AND server='$SERVER_NAME';" | tail -1)
TOP_IP=$(mysql_exec "SELECT CONCAT(ip,' (',fail_count,' Versuche)') FROM daily_stats WHERE stat_date='$YESTERDAY' AND server='$SERVER_NAME' ORDER BY fail_count DESC LIMIT 1;" | tail -1)

log "Zusammenfassung $YESTERDAY ($SERVER_NAME): $TOTAL_FAILS Fehlversuche von $TOTAL_IPS IPs | Top: ${TOP_IP:-keine}"
